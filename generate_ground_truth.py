"""
Create a ground truth file of listings and owl:sameAs links between the
listings.
"""
import argparse
import csv
...
def main() -> None:
    ...
    with open(args.input_labels, "r") as input_labels:
        with open(args.output_labels, "w") as output_labels:
            ...
            duplicates, uniques = select_labels(labels)
...
def format_non_duplicates(duplicates: list[list[str]], uniques: list[str]) -> Iterator[list]:
    """
    Given a list of duplicates like the following:
        [
            [ A, B, C ],
...
def select_labels(labels: list[list[str]]) -> tuple[list[list[str]], list[str]]:
    """
    ...
    Remove some of the duplicates (randomly) to ensure the output
    ...
    uniques: list[str] = [random.choice(dups) for dups in labels[len(labels) // 2 :]]
...
def read_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-labels", default="input/labels.csv", help="Input labels file", type=str
    )
    parser.add_argument(
        "--input-data",
